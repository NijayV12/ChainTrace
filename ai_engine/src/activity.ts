...
export function computeSuspiciousLoginScore(
  activityLogs: LoginActivitySignal[],
  now = Date.now()
): number {
  if (activityLogs.length === 0) return 20;

  const distinctIps = new Set(
    activityLogs.map((log) => log.ipAddress?.trim()).filter(Boolean)
  ).size;
  const distinctDevices = new Set(
    activityLogs.map((log) => log.device?.trim()).filter(Boolean)
  ).size;
