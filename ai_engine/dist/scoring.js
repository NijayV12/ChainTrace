...
function accountAgeScore(accountAgeMonths) {
...
function profileCompletenessScore(profileComplete) {
    return profileComplete ? 100 : 0;
}
function followerRatioScore(followers, following) {
    if (following === 0)
        return followers > 0 ? 100 : 50;
...
function postingPatternScore(posts) {
    if (posts >= 100)
        return 100;
    if (posts >= 50)
        return 90;
...
function profileRiskScore(profileComplete) {
    return profileComplete ? 0 : 70;
}
function networkRiskScore(followers, following) {
    if (followers === 0 && following === 0)
        return 85;
    if (following === 0)
        return followers > 0 ? 10 : 75;
    const ratio = followers / following;
    if (followers < 10 && following > 200)
        return 85;
    if (ratio < 0.1)
        return 80;
    if (ratio < 0.25)
    ...
    if (ratio < 0.5)
        return 45;
    if (ratio > 8 && followers < 100)
        return 40;
    return 15;
}
function activityRiskScore(posts, accountAgeMonths) {
    if (posts === 0)
        return 85;
    const postsPerMonth = posts / Math.max(accountAgeMonths, 1);
    if (postsPerMonth > 200)
        return 90;
    if (postsPerMonth > 100)
        return 70;
    if (postsPerMonth > 40)
        return 45;
    if (postsPerMonth < 0.5 && accountAgeMonths > 12)
        return 35;
    return 15;
}
export function computeTrustScore(input) {
    ...
    const credibilityBoost = 0.22 * accountAgeScore(input.accountAgeMonths) +
