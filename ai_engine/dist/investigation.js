import { classifyFakeScore, } from "./fakeScoring.js";
import { classifyScore } from "./scoring.js";
import { computeSuspiciousLoginScore, } from "./activity.js";
function parseMLTopFeatures(raw) {
    if (!raw)
        return [];
    try {
...
function parseAnomalySignals(raw) {
    if (!raw)
        return [];
    try {
        const parsed = JSON.parse(raw);
...
export function estimateUsernameRandomness(handle) {
...
export function extractFakeFeatures(account, activity, now = Date.now()) {
    ...
    const distinctIps = new Set(activity.map((entry) => entry.ipAddress?.trim()).filter(Boolean)).size;
    return {
        ...
        followerFollowingRatio: account.following === 0
            ? account.followers > 0
                ? account.followers
                : 0
...
export function parseLinkedProfiles(raw) {
    if (!raw)
        return [];
    try {
        const parsed = JSON.parse(raw);
...
export function buildAccountInvestigationSummary(account, activityLogs) {
    const riskSignals = [];
    ...
    if (account.accountAge >= 24)
        credibilitySignals.push("Long account age improves trust.");
    else if (account.accountAge < 3)
        riskSignals.push("Very new account age increases fraud risk.");
    if (account.profileComplete)
        credibilitySignals.push("Profile appears complete.");
    else
        riskSignals.push("Incomplete profile reduces authenticity confidence.");
    ...
    if (account.posts === 0)
        riskSignals.push("No posting history available for corroboration.");
    else if (account.posts >= 50)
        credibilitySignals.push("Posting history is substantial enough for review.");
    ...
    if (loginRisk >= 70)
        riskSignals.push("Login activity pattern is operationally suspicious.");
    else if (loginRisk <= 30)
        credibilitySignals.push("Login activity pattern looks relatively stable.");
    ...
    return {
        baseClassification: account.trustScore != null ? classifyScore(account.trustScore) : "PENDING",
        fakeProfileClassification: account.fakeTrustScore != null
        ...
        linkedProfiles,
        recommendedAction: riskSignals.length >= 3
