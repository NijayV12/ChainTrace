...
async function callPythonInference(body, suspiciousLoginScore, ipDeviceVariation) {
    if (!pythonServiceUrl)
        return null;
...
const server = http.createServer(async (req, res) => {
    if (req.method === "GET" && req.url === "/health") {
        const body = { status: "ok", service: "ai_engine" };
    ...
    if (req.method === "POST" && req.url === "/v1/evaluate-account") {
        try {
            ...
            const result = evaluateAccountRiskLocal({
                account: {
                    ...body.account,
            ...
            const pythonAssessment = await callPythonInference(body, result.suspiciousLoginScore, Math.max(result.fakeFeatures.loginLocationVariation, result.fakeFeatures.deviceFingerprintRisk));
            const finalResult = pythonAssessment == null
                ? result
                : {
                    ...result,
    ...
    if (req.method === "POST" && req.url === "/v1/summarize-account") {
        try {
            ...
            const result = buildAccountInvestigationSummary(account, parseDateLogs(body.activityLogs));
