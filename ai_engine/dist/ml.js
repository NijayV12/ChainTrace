...
function normalizeFollowers(followers, following) {
    if (following <= 0)
        return followers > 0 ? 0.2 : 0.7;
...
function normalizePosting(posts, accountAgeMonths) {
    if (posts === 0)
        return 0.85;
    const postsPerMonth = posts / Math.max(accountAgeMonths, 1);
    if (postsPerMonth > 120)
        return 0.82;
    if (postsPerMonth > 60)
        return 0.66;
    if (postsPerMonth > 20)
...
export function inferFraudRisk(input) {
...
export function inferAnomalyRisk(input) {
    const signals = [
        {
    ...
    const anomalyScore = Number((0.3 * signals[0].score +
        0.2 * signals[1].score +
        0.15 * signals[2].score +
        0.2 * signals[3].score +
        0.15 * signals[4].score).toFixed(4));
    return {
        anomalyScore,
